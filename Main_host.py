...
class myMainClass():
    def __init__(self):
        ...
        GUI.config_radioButton_ccOn.toggled.connect(self.config_radioButton_ccOnOff)

        GUI.config_pushButton_copy.clicked.connect(self.config_pushButton_copy)
        GUI.config_pushButton_paste.clicked.connect(self.config_pushButton_paste)
        GUI.config_pushButton_remove.clicked.connect(self.config_pushButton_remove)

        GUI.config_tableWidget_individualGroups.setSelectionBehavior(QtWidgets.QTableView.SelectRows)
        GUI.config_tableWidget_individualGroups.setColumnWidth(1, 160)
    ...
    def tick2(self):
        ...
        rows = sorted(set(index.row() for index in GUI.config_tableWidget_individualGroups.selectedIndexes()))
        if rows and rows != self.rows_prev:
            self.rows_prev = rows
            id = GUI.config_tableWidget_individualGroups.item(rows[0], 0).text()
            self.load_camera_configs(id)

        elif rows != self.rows_prev:
            self.rows_prev = rows
            self.load_camera_configs('nothing')

        if rows:
            GUI.config_tableWidget_cameraConfN.setEnabled(True)
            GUI.config_tableWidget_cameraConfP.setEnabled(True)
            GUI.config_tableWidget_delayConf.setEnabled(True)
            GUI.config_pushButton_cameraConf.setEnabled(True)
            GUI.config_pushButton_copy.setEnabled(True)
            GUI.config_pushButton_paste.setEnabled(True)
            GUI.config_pushButton_remove.setEnabled(True)
        else:
            GUI.config_tableWidget_cameraConfN.setEnabled(False)
            GUI.config_tableWidget_cameraConfP.setEnabled(False)
            GUI.config_tableWidget_delayConf.setEnabled(False)
            GUI.config_pushButton_cameraConf.setEnabled(False)
            GUI.config_pushButton_copy.setEnabled(False)
            GUI.config_pushButton_paste.setEnabled(False)
            GUI.config_pushButton_remove.setEnabled(False)



    def load_camera_configs(self, camID):
        global Camera_configs
        try:
            cam_config = Camera_configs.get(camID)

            try:
                conf_NP = cam_config.get('camera')
                conf_Delay = cam_config.get('delay')
            except:
                pass
            ...
            for i in range(1, GUI.config_tableWidget_cameraConfN.rowCount()):
                try:
                    ...
                    if cam_config:
                        GUI.config_tableWidget_cameraConfN.setItem(i, 0, QtWidgets.QTableWidgetItem(conf_NP.get(header)[0]))
                    else:
                        GUI.config_tableWidget_cameraConfN.setItem(i, 0, QtWidgets.QTableWidgetItem(''))
                ...
                try:
                    ...
                    if cam_config:
                        GUI.config_tableWidget_cameraConfP.setItem(i, 0, QtWidgets.QTableWidgetItem(conf_NP.get(header)[1]))
                    else:
                        GUI.config_tableWidget_cameraConfP.setItem(i, 0, QtWidgets.QTableWidgetItem(''))
            ...
            for i in range(GUI.config_tableWidget_delayConf.rowCount()):
                try:
                    ...
                    if cam_config:
                        GUI.config_tableWidget_delayConf.setItem(i, 0, QtWidgets.QTableWidgetItem(conf_Delay.get(header)))
                    else:
                        GUI.config_tableWidget_delayConf.setItem(i, 0, QtWidgets.QTableWidgetItem(''))
    ...
    def config_groupTable_update(self):
        global Expected_IDs

        eids = Expected_IDs.copy()
        eids.sort()
        eids = ['Default'] + eids

        rows = len(eids)

        GUI.config_tableWidget_individualGroups.setRowCount(rows)
        for n in range(len(eids)):
            GUI.config_tableWidget_individualGroups.setItem(n, 0, QtWidgets.QTableWidgetItem(eids[n]))
            stts = Camera_configs.get(eids[n])
            if stts:
                if eids[n] == 'Default':
                    stts = 'Default'
                else:
                ...
                stts = 'Default'
            GUI.config_tableWidget_individualGroups.setItem(n, 1, QtWidgets.QTableWidgetItem(str(stts)))

            # if 'Not Connected' in Conns[n][1] and (m == 0 or m == 0):
            #     GUI.config_tableWidget_individualGroups.item(n, m).setBackground(QtGui.QColor(255, 150, 0))
            # if 'Failed' in Conns[n][5]:
            #     GUI.config_tableWidget_individualGroups.item(n, 5).setBackground(QtGui.QColor(255, 100, 0))
            # if 'Failed' in Conns[n][6]:
            #     GUI.config_tableWidget_individualGroups.item(n, 6).setBackground(QtGui.QColor(255, 100, 0))
    ...
    def config_pushButton_cameraConf(self):
        ...
        cam_config.update({'camera': conf_NP})
        cam_config.update({'delay': conf_Delay})
        ...
        rows = sorted(set(index.row() for index in GUI.config_tableWidget_individualGroups.selectedIndexes()))
        for row in rows:
            id = GUI.config_tableWidget_individualGroups.item(row, 0).text()
            Camera_configs.update({id:cam_config})
    ...
    def config_radioButton_ccOnOff(self):
        self.config_pushButton_cameraConf()


    def config_pushButton_copy(self):
        global Camera_configs

        rows = sorted(set(index.row() for index in GUI.config_tableWidget_individualGroups.selectedIndexes()))
        if rows:
            id = GUI.config_tableWidget_individualGroups.item(rows[0], 0).text()
            cam_config = Camera_configs.get(id)
            Camera_configs.update({'copy': cam_config})
            self.conf_status_update('Frofile copied.')




    def config_pushButton_paste(self):
        global Camera_configs

        cam_config = Camera_configs.get('copy')
        if cam_config:
            self.load_camera_configs('copy')




    def config_pushButton_remove(self):
        global Camera_configs

        con = pymsgbox.confirm('Remove selected profiles ?','Remove confirmation !')
        if con != 'OK':
            return
        ...
        self.conf_status_update('Selected profiles removed.')
        self.config_groupTable_update()
    ...
    ##################### imran #########################
    def codeupload(self):
        threading.Thread(target=self.codeupload1, daemon=True, args=[]).start()

    def codeupload1(self):
        try:
            GUI.pushButton_upload.setEnabled(False)
            ...
            for th in Threads:
                ...
                try:
                    ...
                    print('Upload failed.................')
            upload_q.put("Request List length----" + str(len(uploadList)))     
            tTime = time.time()  
            while uCount!=0 and time.time() - tTime <=10:
                time.sleep(1)
            ...
            print(e, "---- Code upload")
        GUI.pushButton_upload.setEnabled(True)
...
class ClientThread(threading.Thread):
    ...
    def send_cameraConf(self):
        ...
        cam_config = Camera_configs.get(self.name)
        print(self.name, type(self.name))
        if not cam_config:
            cam_config = Camera_configs.get('Default')
            print('Default')

        colorCorrection = str(GUI.config_radioButton_ccOn.isChecked())
        cam_config.update({'colorCorrection': colorCorrection})

        conf_str = json.dumps(cam_config)
    ...
    def projectorController(self, IP):
        ...
        while True:
            ...
            if Capture_now == True:
                ...
                d1 = float(cam_config.get('delay').get('projection_on').strip())
                d2 = float(cam_config.get('delay').get('projection_off').strip())
...
class ImranThread(threading.Thread):
    ...
    def run(self):
        ...
        global downloadList, uCount, upload_q, u_filename, down_q, uploadList
        try:
            print("receiving......")
            rcv = self.client.recv(1024).decode()
            global dCount
                ...
                try:
                    ...
                    with open(u_filename, "r") as f:
                        ...
                        if data == 'done':
                            ...
                            upload_q.put("Done----" + str(rcv))
                            uploadList.remove(rcv)
                    uCount-=1  
                except Exception as e:
                    print(e,"---- file upload")
                    uCount-=1    
